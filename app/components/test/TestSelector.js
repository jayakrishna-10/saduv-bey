...
import { FileText, Edit, Play, Loader2, BookOpen, Clock, Hash, ChevronRight, CheckCircle2 } from 'lucide-react';
...
export function TestSelector({ onStartTest, isLoading }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-4xl bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/50 overflow-hidden">
        <div className="p-8">
            ...
            <AnimatePresence mode="wait">
                <motion.div key={mode} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3, ease: 'easeInOut' }}>
                    {mode === 'mock' ? <MockOptions config={config} setConfig={setConfig} /> : <PracticeOptions config={config} setConfig={setConfig} topics={topics} isTopicsLoading={isTopicsLoading} />}
                </motion.div>
            </AnimatePresence>
        </div>

        <div className="mt-8 border-t border-gray-200/50 dark:border-gray-700/50 p-6 bg-white/50 dark:bg-gray-900/50 flex justify-between items-center">
...
const MockOptions = ({ config, setConfig }) => (
    <div className="space-y-4">
        <h3 className="text-lg font-medium text-center text-gray-900 dark:text-gray-100 mb-4">Select Your Paper</h3>
...
const PracticeOptions = ({ config, setConfig, topics, isTopicsLoading }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4 p-4 bg-white/50 dark:bg-gray-700/50 rounded-xl">
        ...
        <div className="space-y-4 p-4 bg-white/50 dark:bg-gray-700/50 rounded-xl">
            <label htmlFor="questionCount" className="font-medium text-gray-800 dark:text-gray-200">Questions: {config.questionCount}</label>
            <input id="questionCount" type="range" min="5" max="50" step="5" value={config.questionCount} onChange={e => setConfig(prev => ({ ...prev, questionCount: parseInt(e.target.value) }))} className="w-full slider" />
            <div className="text-sm text-gray-600 dark:text-gray-400">Estimated Time: {Math.floor((config.questionCount * 72) / 60)} min { (config.questionCount * 72) % 60} sec</div>
        </div>
    </div>
);

const ModeCard = ({ icon: Icon, title, description, selected, onClick }) => (
    <motion.button onClick={onClick} whileHover={{ y: -5 }} className={`p-6 rounded-2xl text-left transition-all border-2 h-full ${selected ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 shadow-lg' : 'bg-white/50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
...
const PaperCard = ({ paper, selected, onClick }) => (
    <motion.button onClick={onClick} whileHover={{ y: -5 }} className={`p-6 rounded-2xl text-center transition-all border-2 relative h-full flex flex-col justify-center items-center ${selected ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 shadow-lg' : 'bg-white/50 dark:bg-gray-700/50 border-gray-200 dark:border-gray-700 hover:border-gray-300 dark:hover:border-gray-600'}`}>
