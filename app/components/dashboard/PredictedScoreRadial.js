...
export function PredictedScoreRadial({ analytics }) {
  ...
  const getScoreColor = (score) => {
    if (score >= 40) return '#10b981'; // green - 80%+ 
    if (score >= 30) return '#3b82f6'; // blue - 60%+
    if (score >= 20) return '#f59e0b'; // amber - 40%+
    return '#ef4444'; // red - below 40%
  };

  const getGradientColors = (score) => {
    if (score >= 40) return 'from-emerald-400 to-green-500';
    if (score >= 30) return 'from-blue-400 to-indigo-500';
    if (score >= 20) return 'from-amber-400 to-orange-500';
    return 'from-red-400 to-pink-500';
  };

  const getPaperConfig = (paperKey) => {
    const configs = {
      paper1: {
        name: 'Paper 1',
  ...
  const PaperCircle = ({ paperKey, index }) => {
    const config = getPaperConfig(paperKey);
    const percentage = predictedScores[paperKey];
    const marks = Math.round((percentage / 100) * 50); // Convert to marks out of 50
    ...
    return (
      <motion.div
        ...
        className={`relative p-4 rounded-xl bg-gradient-to-br ${config.bgColor} border border-gray-200 dark:border-gray-600 hover:shadow-lg transition-all cursor-pointer`}
      >
        <div className="flex flex-col items-center">
          ...
          <motion.div
            ...
            className={`mt-3 px-3 py-1 rounded-full text-xs font-medium bg-gradient-to-r ${config.color} text-white`}
          >
        ...
        {hoveredPaper === paperKey && (
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1 }}
            className="absolute -top-2 left-1/2 transform -translate-x-1/2 -translate-y-full p-2 bg-gray-900 text-white text-xs rounded-lg shadow-xl z-10 pointer-events-none whitespace-nowrap"
          >
            <div className="text-center">
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 rounded-lg">
        <div className="text-center">
          ...
          <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            {Math.round((predictedScores.paper1 + predictedScores.paper2 + predictedScores.paper3) / 100 * 150)} / 150
          </div>
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs text-indigo-800 dark:text-indigo-200">
            {predictedScores.overall < 50 
              ...
              ? "Great progress! Target your weakest chapters to boost performance."
              : "Excellent work! Fine-tune weak areas to achieve top scores."}
          </div>
        </div>
      </div>
