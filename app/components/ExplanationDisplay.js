// app/components/ExplanationDisplay.js - Comprehensive explanation display for quiz
'use client';
    ...
    return (
      <div className="p-6 bg-yellow-50 border border-yellow-200 rounded-2xl">
        <div className="flex items-center gap-3">
          ...
          <p className="text-yellow-800">Explanation not available for this question.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              {explanation.correct_answer?.supporting_facts && (
                <ul className="mt-3 text-indigo-800 text-sm space-y-1">
                  {explanation.correct_answer.supporting_facts.map((fact, index) => (
                    <li key={index} className="flex items-start gap-2">
                      <span className="text-indigo-500 mt-1.5 w-1 h-1 rounded-full bg-current flex-shrink-0" />
                      {fact}
                    </li>
                  ))}
                </ul>
            ...
            >
              <div className="flex items-start gap-3">
                <div className="flex-shrink-0 flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-white/70 flex items-center justify-center font-medium">
                    {option.toUpperCase()}
                  </div>
            ...
            <AnimatePresence>
              {expandedSections.technical && (
                <motion.div
                  ...
                  className="mt-4 p-6 bg-white/50 rounded-2xl border border-gray-200/50"
                >
                  {explanation.technical_details.formulas?.map((formula, index) => (
                    <div key={index} className="mb-4 last:mb-0">
                      <div className="bg-gray-900 text-white p-4 rounded-xl font-mono text-sm mb-2">
                        <ReactMarkdown
                      ...
                      <p className="text-gray-700 text-sm mb-2">{formula.description}</p>
                      {formula.variables && (
                  ...
                  {explanation.technical_details.specifications && (
                    <div className="mt-4 p-4 bg-blue-50 rounded-xl">
                      <h5 className="font-medium text-blue-900 mb-3">Specifications</h5>
                      ...
                      {explanation.technical_details.specifications.standards && (
                        <div className="mt-3">
                          <h6 className="font-medium text-blue-900 text-sm mb-2">Standards:</h6>
                  ...
                  {explanation.technical_details.process_description && (
                    <div className="mt-4 p-4 bg-gray-50 rounded-xl">
                      <h5 className="font-medium text-gray-900 mb-2">Process Description</h5>
                ...
                >
                  {explanation.visual_aids.tables?.map((table, index) => (
                    <div key={index} className="mb-6 last:mb-0">
                      <h5 className="font-medium text-gray-900 mb-3">{table.title}</h5>
                  ...
                  {explanation.visual_aids.diagrams?.map((diagram, index) => (
                    <div key={index} className="mb-4 last:mb-0">
                      <h5 className="font-medium text-gray-900 mb-2">{diagram.title}</h5>
              ...
              {expandedSections.practical && (
                ...
                >
                  {explanation.practical_applications.map((app, index) => (
                    <div key={index} className="mb-4 last:mb-0 p-4 bg-green-50 rounded-xl border border-green-200">
                      <h5 className="font-medium text-green-900 mb-2">{app.industry}</h5>
                      <p className="text-green-800 text-sm mb-2">{app.use_case}</p>
                      <p className="text-green-700 text-sm italic">{app.example}</p>
                    </div>
                  ))}
                </motion.div>
              )}
                ...
                >
                  {explanation.memory_aids && (
                    <div className="mb-4 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
                      <h5 className="font-medium text-yellow-900 mb-2">Memory Aids</h5>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {explanation.study_tips.common_mistakes && (
                      <div>
                        <h5 className="font-medium text-gray-900 mb-2">Common Mistakes</h5>
                        <ul className="list-disc list-inside text-sm text-gray-700 space-y-1">
                  ...
                  {explanation.study_tips.exam_strategy && (
                    <div className="mt-4 p-4 bg-blue-50 rounded-xl border border-blue-200">
                      <h5 className="font-medium text-blue-900 mb-2">Exam Strategy</h5>
                      <p className="text-blue-800 text-sm">{explanation.study_tips.exam_strategy}</p>
                    </div>
                  )}
                </motion.div>
              )}
      ...
      {explanation.related_topics && explanation.related_topics.length > 0 && (
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50">
          <h4 className="font-medium text-gray-900 mb-4">Related Topics</h4>
          <div className="space-y-3">
            {explanation.related_topics.map((topic, index) => (
              <div key={index} className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">{topic.relationship}</p>
      ...
      {/* Voting */}
      <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-6 border border-gray-200/50">
        <h4 className="font-medium text-gray-900 mb-4">Was this explanation helpful?</h4>
